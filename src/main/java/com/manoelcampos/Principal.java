...
public class Principal {
    public static void main(String[] args) {
        ...
        newsletter.setDefaultMessageService(new Sms());
        newsletter.send(msgTemplate);

        newsletter.setDefaultMessageService(new Email());
        customers.stream().filter(customer->customer.getName()=="Manoel").findFirst().ifPresentOrElse((customer)->customer.setMessageService(new WhatsApp()),()->{});
