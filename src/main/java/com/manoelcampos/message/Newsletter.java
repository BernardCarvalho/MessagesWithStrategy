...
public class Newsletter {
    ...
    public Newsletter(final List<Customer> customers){
        ...
        this.defaultMessageService = new WhatsApp();
    }
    ...
    public void send(final String msgTemplate) {
        for (final Customer customer : customers) {
            MessageService messageServiceToBeUsed = (customer.getMessageService()==null?defaultMessageService:customer.getMessageService());
            messageServiceToBeUsed.send(customer.getPhone(), formatMsg(customer, msgTemplate));
        }
    }
